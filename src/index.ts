...
const GENERATED_HEADER = `# ⚠️ Generated by aistack
# Do not edit manually.
# Edit .ai/* and run: aistack sync
`;
...
program
  ...
  .description("Modo mágico: configura el repo automáticamente")
...
program
  ...
  .action(async (projectName?: string) => {
    ...
    console.log("✅ Estructura .ai/ creada o verificada");
  });
...
program
  .command("add")
  .description("Agrega/genera configuración para una herramienta AI")
  ...
  .action(async (tool: string, opts: { force: boolean }) => {
    ...
    if (!(await fs.pathExists(aiPath))) {
      console.log("⚠️ No existe .ai/, creando estructura base...");
      await createBaseAI(cwd);
    } else {
    ...
    console.error("❌ Tool no soportada. Usa: cursor | claude");
    process.exit(1);
  });
...
program
  ...
  .option("--force", "Sobrescribe adapters existentes aunque no sean de aistack", false)
  .action(async (tool: "cursor" | "claude" | "both", opts: { force: boolean }) => {
    ...
    console.log("🔄 Sincronizando adapters...\n");
...
async function runMagicInteractive(cwd: string) {
  ...
  if (!repo.hasAiDir && !repo.hasCursorRules && !repo.hasClaudeMd) {
    const wants = await confirm({
      message: "No tienes nada configurado. ¿Quieres configurar un AI workspace ahora?",
      default: true,
    });

    if (!wants) {
      console.log("Listo. Cuando quieras, ejecuta: aistack init");
      return;
    }
  }
  ...
  const tool = await select<"cursor" | "claude" | "both">({
    message: "¿Qué quieres configurar?",
    choices: [
      {
        name: `Cursor (.cursorrules)${recommended === "cursor" ? " — recomendado" : ""}`,
        value: "cursor",
      },
      {
        name: `Claude (CLAUDE.md)${recommended === "claude" ? " — recomendado" : ""}`,
        value: "claude",
      },
      {
        name: `Ambos${recommended === "both" ? " — recomendado" : ""}`,
        value: "both",
      },
    ],
  });
  ...
  console.log("✅ Hecho. Checklist:\n");
  for (const r of results) {
    console.log(`  ${r.ok ? "✅" : "⚠️"} ${r.label}${r.note ? " — " + r.note : ""}`);
  }
  if (created.createdAiDir) console.log("  ✅ Creada carpeta .ai/");
  if (created.bootstrappedContext) console.log("  ✅ Generado/asegurado .ai/context.md");
  if (created.ensuredPlaybooks) console.log("  ✅ Playbooks asegurados (.ai/playbooks/)");
  ...
  console.log("  1) Abre .ai/context.md y escribe el propósito del proyecto en 2–4 líneas");
  console.log("  2) Ajusta .ai/rules.md según tu estilo");
  console.log("  3) Vuelve a correr: aistack sync cursor --force (para regenerar)");
}
...
async function runMagicNonInteractive(
  ...
  if (!before.hasAiDir) {
    console.log("✨ No existe .ai/ → creando estructura base...");
    await createBaseAI(cwd);
  } else {
    await ensureAiMinimum(cwd);
  }
...
async function generateCursorWithOverwritePrompt(
  ...
  if (await fs.pathExists(outPath)) {
    ...
    if (ours && !force) {
      ...
      return { label: "Regenerado .cursorrules (archivo era de aistack)", ok: true };
    }

    if (!force) {
      if (nonInteractive) {
        return {
          label: "No se sobrescribió .cursorrules",
          ok: true,
      ...
      const overwrite = await confirm({
        message: "Ya existe .cursorrules. ¿Quieres sobrescribirlo?",
        default: false,
      });

      if (!overwrite) {
        return { label: "No se sobrescribió .cursorrules", ok: true, note: "lo dejé intacto" };
      }
    }
  }
...
async function generateClaudeWithOverwritePrompt(
  ...
  if (await fs.pathExists(outPath)) {
    ...
    if (ours && !force) {
      ...
      return { label: "Regenerado CLAUDE.md (archivo era de aistack)", ok: true };
    }

    if (!force) {
      if (nonInteractive) {
        return {
          label: "No se sobrescribió CLAUDE.md",
          ok: true,
          note: "ya existe (usa --force para sobrescribir)",
        };
      }
      ...
      const overwrite = await confirm({
        message: "Ya existe CLAUDE.md. ¿Quieres sobrescribirlo?",
        default: false,
      });

      if (!overwrite) {
        return { label: "No se sobrescribió CLAUDE.md", ok: true, note: "lo dejé intacto" };
      }
    }
  }
...
async function isGeneratedByUs(filePath: string) {
  ...
  return head.includes("Generated by aistack") || head.includes("aistack");
}
...
- Mantén el estilo del repo (TypeScript/ESLint/Prettier si existen)
`,
      "utf8"
    );
  }
...
2. Diseñar cambios mínimos (archivos tocados)
3. Implementar
...
- Si falta información: pide el archivo exacto o declara suposiciones mínimas.
`;
...
- Antes de finalizar: sugiere cómo validar (tests/lint/build).
`;
...
function printDoctorReport(repo: RepoInspection, tools: ToolDetection) {
  console.log("\n📁 Repo setup:");
  console.log(`  .ai/ existe: ${repo.hasAiDir ? "✅" : "❌"}`);
