...
program
  ...
  .option("--force", "Sobrescribe adapters existentes aunque no sean de devstackai", false)
...
async function runMagicInteractive(cwd: string) {
  ...
  if (!repo.hasAiDir && !repo.hasCursorRules && !repo.hasClaudeMd) {
    ...
    if (!wants) {
      console.log("Listo. Cuando quieras, ejecuta: devstackai init");
      return;
    }
  }
  ...
  console.log("  3) Vuelve a correr: devstackai sync cursor --force (para regenerar)");
}
...
async function generateCursorWithOverwritePrompt(
  ...
  if (await fs.pathExists(outPath)) {
    ...
    if (ours && !force) {
      ...
      return { label: "Regenerado .cursorrules (archivo era de devstackai)", ok: true };
    }
...
async function generateClaudeWithOverwritePrompt(
  ...
  if (await fs.pathExists(outPath)) {
    ...
    if (ours && !force) {
      ...
      return { label: "Regenerado CLAUDE.md (archivo era de devstackai)", ok: true };
    }
